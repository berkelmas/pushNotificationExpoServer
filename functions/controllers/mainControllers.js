...
const sendNotification = async (req, res) => {
  ...
  const { userIds, pushNotificationMessage, pushNotificationData } = req.body;
  ...
  await (async () => {
    for (let userId of JSON.parse(userIds)) {
  ...
  for (let pushToken of pushTokens) {
    ...
    if (!Expo.isExpoPushToken(pushToken)) {
      ...
      return res.json({
        status: "Does not appear all push tokens to be valid."
      });
    }
    ...
    messages.push({
      ...
      data: JSON.parse(pushNotificationData)
    });
  }
